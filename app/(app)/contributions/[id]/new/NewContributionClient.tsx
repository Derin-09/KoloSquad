...
//     useEffect(() => {
//     async function fetchSquadDetails() {
//       const { data: squad } = await supabase.from("squads").select("target_amount").eq("id", squadId).single()
//       const { data: members } = await supabase.from("squad_members").select("id").eq("squad_id", squadId)
//       if (squad) setSquadTarget(squad.target_amount)
...
//       let periods = 1
//       if (frequency === "weekly") periods = Math.max(1, differenceInWeeks(end, start))
//       else if (frequency === "bi-weekly") periods = Math.max(1, Math.floor(differenceInWeeks(end, start) / 2))
//       else if (frequency === "monthly") periods = Math.max(1, differenceInMonths(end, start))

//       const perMember = squadTarget / memberCount / periods
//       setCalculatedAmount(perMember)
//     }
//   }, [startDate, endDate, squadTarget, memberCount, frequency])
...
//       <div className="pt-4 text-sm text-muted-foreground">
//         <p>
//           Squad Target: <span className="text-foreground font-medium">₦{squadTarget.toLocaleString()}</span>
//         </p>
//         <p>
//           Members: <span className="text-foreground font-medium">{memberCount}</span>
//         </p>
//         {calculatedAmount > 0 && (
//           <p>
//             Estimated Per-Member {frequency} Payment:{" "}
