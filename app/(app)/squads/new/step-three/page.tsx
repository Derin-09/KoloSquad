...
import {
  getSquadDraft,
  patchSquadDraft,
  type StepThreeDraftValues,
} from "@/app/(app)/squads/new/draft-storage";

export const initialStepThreeValues: StepThreeDraftValues = {
  memberCount: 5,
};
...
export default function StepThree() {
  ...
  const { values, setFieldValue, handleSubmit, setValues } = useFormik<StepThreeDraftValues>({
    ...
    onSubmit: () => {
      patchSquadDraft({ currentStep: 4, stepThree: values });
      router.push("/squads/new/step-four");
    },
  });

  useEffect(() => {
    patchSquadDraft({ currentStep: 3, stepThree: values });
  }, [values]);
